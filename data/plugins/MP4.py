...
class MP4(Plugin):
    ...
    def configurate(self, dialog):
        ...
        # checkbutton mp4box 
        checkbutton_dont_optimize = gtk.CheckButton("MP4 nicht mit MP4Box optimieren. Schneller erzeugt, aber nicht so kompatible Dateien")
        dialog.vbox.pack_start(checkbutton_dont_optimize, expand=False)
    ...
    def on_mp4_clicked(self, widget, data=None):
        ...
        def mp4():
            for count, filename in enumerate(filenames):
                ...
                args = [ffmpeg, "-loglevel", "info", "-y", "-i", mkvpass_file, "-vcodec", "copy", '-af', 'volume=volume=' + str(vol), "-vsync", "1", '-async',  '1000',  "-dts_delta_threshold", "100", "-drc_scale", "1.0", "-vf", "fps="+ str(fps), ffmpegpass_file]
                ...
                yield 2, count
                try:
                    p = subprocess.Popen([self.app.config.get_program('mp4box'), "-keep-all","-new", "-packed", "-fps", str(fps), "-add" , ffmpegpass_file, mp4boxpass_file] , stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                except OSError:
        ...
        def loop(state, argument):            
            if state == 0:
                self.gui.main_window.set_tasks_text("Extrahiere Streams aus Inputdatei ... %s/%s" % (str(argument + 1), str(len(filenames))))
            elif state == 1:
                self.gui.main_window.set_tasks_text("MP4 erzeugen ... %s/%s" % (str(argument + 1), str(len(filenames))))
            elif state == 2:
                self.gui.main_window.set_tasks_text("MP4 optimieren (importiere Stream) ...  %s/%s" % (str(argument + 1), str(len(filenames))))
            elif state == 3:
                self.gui.main_window.set_tasks_text("Originaldatei in Mülleimer verschieben ... %s/%s" % (str(argument + 1), str(len(filenames))))
            elif state == 5:
                self.gui.main_window.set_tasks_text("Normalisierungswert berechnen ... %s/%s" % (str(argument + 1), str(len(filenames))))
            elif state == 6:
                self.gui.main_window.set_tasks_text("MP4 optimieren (schreibe MP4) ... %s/%s" % (str(argument + 1), str(len(filenames))))
            else:                
